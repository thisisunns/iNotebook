...
const JWT_SECRET = "unnsisagoodboy$";

//Route 1: create user using : "/api/auth/createuser". no login required
router.post(
  "/createuser",
  [
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        ...
        return res
          .status(400)
          .json({ error: "Please try with correct Credentils" });
      }
      ...
      if (!passwordCompare) {
        ...
        return res
          .status(400)
          .json({ success, error: "Please try with correct Credentils" });
      }
      const data = {
        user: {
          id: user.id,
